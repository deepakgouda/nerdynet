...
## Who can issue ROAs?
If an IP address block was directly allocated to Org A and it further assigns a part of the block to Org B, then who can issue RPKI certificates for the smaller address block - Org A or Org B? The answer to this question depends on the delegation status of the address block. The following table summarizes the delegation status of an IP address block and the organization that can issue ROAs for the address block.
